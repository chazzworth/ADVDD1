...
router.post('/campaigns/:id/image', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        // 2. Generate Image with Google (Gemini 3 Pro Image)
        // User requested: "gemini-3-pro-image-preview" (Nano Banana Pro)
        // Uses 'generateContent' API, not 'predict'
        const url = `https://generativelanguage.googleapis.com/v1beta/models/gemini-3-pro-image-preview:generateContent?key=${googleKey}`;
        ...
        if (!candidate) throw new Error("No candidates returned by Gemini 3");

        // Look for image part
        const imagePart = candidate.content.parts.find(p => p.inlineData);

        if (!imagePart) {
            // Check if it refused and returned text
            const textPart = candidate.content.parts.find(p => p.text);
            if (textPart) throw new Error(`Gemini Refused: ${textPart.text.substring(0, 100)}...`);
            throw new Error("No image data returned by Gemini 3");
        }
